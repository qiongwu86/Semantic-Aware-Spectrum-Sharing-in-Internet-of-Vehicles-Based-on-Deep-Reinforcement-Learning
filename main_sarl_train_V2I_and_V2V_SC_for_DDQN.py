...
import Environment_marl_V2I_and_V2V_SC_for_DDQN
import os
from replay_memory_DDQN import ReplayMemory
import sys
...
label = 'sarl_model_for_DDQN_with_sc/lamda0.9_lr0.001_newpowerlist'

n_veh = 4
n_neighbor = 1
n_RB = n_veh

env = Environment_marl_V2I_and_V2V_SC_for_DDQN.Environ(down_lanes, up_lanes, left_lanes, right_lanes, width, height, n_veh, n_neighbor)
env.new_random_game()  # initialize parameters in env

n_episode = 3000
n_step_per_episode = int(env.time_slow/(env.time_fast * 4))
epsi_final = 0.02
...
def get_state(env, idx=(0,0), ind_episode=1., epsi=0.02):
    ...
    return np.concatenate((V2I_fast, np.reshape(V2V_fast, -1), V2V_interference, V2V_SNR_normalized, np.asarray([V2I_abs]), V2V_abs, time_remaining, load_remaining, np.reshape(V2V_symbol, -1), np.reshape(V2I_symbol, -1), np.asarray([ind_episode, epsi])))
...
n_output = n_RB * len(env.V2V_power_dB_List) * 20
...
def predict(sess, s_t, ep, test_ep = False):
    ...
    if np.random.rand() < ep and not test_ep:
        pred_action = np.random.randint(n_RB*n_power_levels*20)
    else:
...
if IS_TRAIN: # 所有车一起训练
    ...
    for i_episode in range(n_episode):
        ...
        #     for i in range(n_RB):
        #         for j in range(n_neighbor):
        #             action = predict(sess, np.asarray(state_old_all).flatten(), epsi)
        #             action_all.append(action)
        #             action_all_training[i, j, 0] = action % n_RB  # chosen RB
        #             action_all_training[i, j, 1] = int(np.floor(action / (n_RB * 20)))  # power level
        #             action_all_training_length[i, 0] = action % 20 + 1
        #             action_all_training_length[i, 1] = int(np.floor(action / (n_RB * 23))) + 1
        #             # print(action_all_training_length[i, 0])
        #             # print(action_all_training_length[i, 1])
        for i_step in range(n_step_per_episode):
            state_old_all = []
            time_step = i_episode * n_step_per_episode + i_step
            for i in range(n_RB):
            ...
            num_states = len(state_old_all)  # 总共的状态数量
            num_states_per_split = num_states // 4  # 每份的状态数量

            action_all = []  # 存储四个不同的 action

            for i in range(n_RB):  # 循环四次，分别处理四份状态
                start_idx = i * num_states_per_split  # 起始索引
                end_idx = (i + 1) * num_states_per_split  # 结束索引

                # 获取当前份的状态
                current_states = state_old_all[start_idx:end_idx]

                for state in current_states:
                    action = predict(sess, np.asarray(state).flatten(), epsi)
                    action_all.append(action)

                    # 计算索引
                    idx = len(action_all) -1

                    # 根据索引将 action 拆分并存储到 action_all_training
                    i_rb, i_neighbor = divmod(idx, n_neighbor)
                    action_all_training[i_rb, i_neighbor, 0] = action % n_RB  # chosen RB
                    action_all_training[i_rb, i_neighbor, 1] = int(np.floor(action / (n_RB * 20)))  # power level
                    action_all_training_length[i_rb, 0] = action % 20 + 1
                    action_all_training_length[i_rb, 1] = int(np.floor(action / (n_RB * 23))) + 1


            action_temp = action_all_training.copy()
            env.rennew_symbols_of_word(action_all_training_length)  # update channel fast fading
            ...
            for i in range(n_RB):
                for j in range(n_neighbor):
                    ...
                    action_lala = action_all[n_neighbor * i + j]
                    state_new = state_new_all[n_neighbor * i + j]
                    agent.memory.add(state_old, state_new, train_reward, action_lala)
    ...
    np.save('Data/DDQN_with_lamda0.9_lr0.001_newpowerlist', reward_average_list)

    current_dir = os.path.dirname(os.path.realpath(__file__))
    reward_path = os.path.join(current_dir, "model/" + label + '/reward.mat')
    scipy.io.savemat(reward_path, {'reward': reward_average_list})
