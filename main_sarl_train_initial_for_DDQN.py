...
label = 'sarl_model_for_DDQN_without_sc'
...
with g.as_default():
    ...
    optim = tf.train.RMSPropOptimizer(learning_rate=0.00001, momentum=0.95, epsilon=0.01).minimize(g_loss)
...
if IS_TRAIN:
    ...
    record_loss = []
    reward_episode = 0  # 用于跟踪当前周期的总奖励
    reward_average_list = []  # 存储每个周期的平均奖励
    ...
    for i_episode in range(n_episode):
        ...
        average_reward = reward_episode / n_step_per_episode  # 计算平均奖励
        print(' Reward ', time_step, ':', average_reward)
        reward_average_list.append(average_reward)
        reward_episode = 0  # 重置周期内的总奖励
    ...
    np.save('Data/DDQN_without_sc2000_learning_rate_0.00001.npy', reward_average_list)
