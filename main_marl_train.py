...
import Environment_marl_initial_for_DDQN
import os
...
env = Environment_marl_initial_for_DDQN.Environ(down_lanes, up_lanes, left_lanes, right_lanes, width, height, n_veh, n_neighbor)
...
def get_state(env, idx=(0,0), ind_episode=1., epsi=0.02):
    ...
    V2V_SNR_normalized = (V2V_SNR + 10) / 40
    ...
    return np.concatenate((V2I_fast, np.reshape(V2V_fast, -1), V2V_interference, V2V_SNR_normalized,
                           np.asarray([V2I_abs]), V2V_abs, time_remaining, load_remaining,
                           np.asarray([ind_episode, epsi])))
...
if IS_TRAIN:
    for i_episode in range(n_episode):
        ...
        if i_episode%100 == 0:
            ...
            env.rennew_symbols_of_word()  # update channel fast fading
    ...
    current_dir = os.path.dirname(os.path.realpath(__file__))
    reward_path = os.path.join(current_dir, "model/" + label + '/reward_1200.mat')
    scipy.io.savemat(reward_path, {'reward': reward_average_list})

    record_loss = np.asarray(record_loss).reshape((-1, n_veh*n_neighbor))
    loss_path = os.path.join(current_dir, "model/" + label + '/train_loss_1200.mat')
