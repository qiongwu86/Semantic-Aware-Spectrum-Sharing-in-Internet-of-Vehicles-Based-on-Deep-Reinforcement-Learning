...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, n_neighbor):
        ...
        # self.bandwidth = 1500
        self.byte_time = 10
        self.demand_size = int((4 * 190 + 300) * self.byte_time)  # V2V payload: 1060 Bytes every 100 ms
        # self.demand_size = 20
            ...
            for j in range(self.n_neighbor):
                ...
                V2I_Interference[actions[i][j]] += 10 ** ((self.V2V_power_dB_List[power_selection[i, j]] -
                                                           self.V2I_channels_with_fastfading[i, actions[
                                                               i, j]] + self.vehAntGain + self.bsAntGain - self.bsNoiseFigure) / 10)
    ...
    def act_for_training(self, actions):
        ...
        # reward =  np.sum(V2I_Rate) / (self.n_Veh * 10) +  np.sum(reward_elements) / (self.n_Veh * self.n_neighbor)
        # print(np.sum(V2I_Rate) / (self.n_Veh * 10))
        # print(np.sum(reward_elements) / (self.n_Veh * self.n_neighbor))
