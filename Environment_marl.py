...
np.random.seed(1234)
# 确保在每次运行程序时使用NumPy库生成生成的随机数序列是可重复的
symbols_of_word = np.load('symbols_of_word.npy')
# 加载 "table_data.mat" 文件
data = sio.loadmat('table_data.mat')
# 获取 'table_data' 对应的数组
...
def calculate_snr(signal_powers, interference_powers):
    snr_list = []
    for signal_power, interference_power in zip(signal_powers, interference_powers):
        ...
        # 将信噪比限制在 -10 dB 到 30 dB 的范围内
        snr_db = max(-185, min(60, snr_db))
        snr_list.append(snr_db)

    return snr_list


def scale_snr(snr_values, min_snr, max_snr, low=-10, high=30):
    snr_values = np.array(snr_values)  # 将snr_values转换为numpy数组
    # 计算比例
    scaled_snr_values = low + (snr_values - min_snr) * (high - low) / (max_snr - min_snr)

    return scaled_snr_values
...
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh, n_neighbor):
        ...
        self.V2V_symbols_of_word = np.zeros((len(self.vehicles)))
        self.snr_results = np.full((4, 1), -10)
...
# 对角线元素表示车辆与自身的路径损耗，设置为50，表示较高的路径损耗
    ...
    def rennew_symbols_of_word(self):
        """ Renew symbols of every word """
        self.V2V_symbols_of_word = []
        # 遍历每辆车的 SNR
        for snr in self.snr_results:
            # 获取对应 SNR 的句子长度列表
            selected_lengths = symbols_of_word[snr + 10]
            # 随机选择一个句子长度
            selected_length = np.random.choice(selected_lengths[0])
            self.V2V_symbols_of_word.append(selected_length)
        # 将 self.V2V_symbols_of_word 转换为 NumPy 数组
        self.V2V_symbols_of_word = np.array(self.V2V_symbols_of_word)
        # 输出结果
        # print("每辆车选择的句子长度列表：", self.V2V_symbols_of_word)
    ...
    def Compute_Performance_Reward_Train(self, actions_power):
        ...
        # 计算每个信号的信噪比
        snr_results_V2I_train = calculate_snr(V2I_Signals, self.V2I_Interference)
        # 将snr_results_V2I_train转换为Numpy数组
        snr_results_V2I_train = np.array(snr_results_V2I_train)
        # print("训练过程中V2I信噪比为:", snr_results_V2I_train, "dB")
        V2I_SC_SSE = snr_results_V2I_train / self.u
        V2I_SC_SR = V2I_SC_SSE * self.bandwidth
        # print('V2I_SC_SR(suits/s)', V2I_SC_SR)
        ...
        # 计算每个信号的信噪比
        snr_results_V2V_train = calculate_snr(V2V_Signal, self.V2V_Interference)
        # print("训练过程中V2V信噪比为:", snr_results_V2V_train, "dB")
        self.snr_results_list.append(snr_results_V2V_train)
        ...
        max_snr = 37
        # 放缩数据
        scaled_snr_results = scale_snr(snr_results_V2V_train, min_snr, max_snr)
        # 将 scaled_snr_results 转换为整数
        scaled_snr_results = scaled_snr_results.astype(int)
        # print(scaled_snr_results)

        self.snr_results = np.array(scaled_snr_results).reshape(4, 1)

        semantic_similarity = []  # 存储每辆车的语义相似度

        # 遍历每辆车的句子长度和信噪比
        for length, snr in zip(self.V2V_symbols_of_word, self.snr_results):
            # 找到对应的行索引
            row_index = length - 3  # 因为句子长度是从 3 开始的，而数组索引是从 0 开始的
            if row_index >= table_data.shape[0]:
                row_index = table_data.shape[0] - 1

            # 找到对应的列索引
            col_index = snr + 11  # 因为 SNR 是从 -10 到 30，而数组索引是从 0 到 40

            # 获取对应的语义相似性值，并添加到列表中
            semantic_similarity.append(table_data[row_index, col_index])
            # print("Current SNR:", snr, "Current Length:", length, "Semantic Similarity:",
            #       table_data[row_index, col_index])

        # 将列表转换为 NumPy 数组，并调整形状为 (4, 1)
        semantic_similarity = np.array(semantic_similarity).reshape((4, 1))
        V2V_symbols_new = self.V2V_symbols_of_word.reshape((4, 1))

        # 计算 V2V_SC_SSE，即相除得到的结果
        V2V_SC_SSE = semantic_similarity / V2V_symbols_new
        ...
        # print('V2V_SNR' ,np.log10(np.divide(V2V_Signal, self.V2V_Interference)) )
        ...
        return V2I_SC_SR, V2V_SC_SSE, reward_elements
    ...
    def compute_V2V_SNR(self, actions):
        ...
        max_snr = 40
        # 放缩数据
        self.V2V_SNR_all_dB = scale_snr(self.V2V_SNR_all_dB, min_snr, max_snr)
        # print(self.V2V_SNR_all_dB)
    ...
    def act_for_training(self, actions):
        ...
        V2I_SC_SR, V2V_SC_SSE, reward_elements = self.Compute_Performance_Reward_Train(action_temp)
        ...
        reward =  np.sum(V2I_SC_SR) / (self.n_Veh *  int(1e6)) +  np.sum(reward_elements) / (self.n_Veh * self.n_neighbor)

        print('第一部分奖励' , np.sum(V2I_SC_SR) / (self.n_Veh  * int(1e6)))
        print('第一部分奖励' ,np.sum(reward_elements) / (self.n_Veh * self.n_neighbor))
        print('总体的奖励' , reward)
        return reward
